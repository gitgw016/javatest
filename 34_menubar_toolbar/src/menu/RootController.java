...
public class RootController implements Initializable {
	...
	protected void handleDirectoryChooser() {
		...
		if(selectedDir != null) {
			// directory 안에 존재하는 파일 or dir(폴더)정보를 출력
	...
	protected void handlePopup() {
		...
		Image image = new Image(getClass().getResource("../images/dialog-info.png").toString());
		imageView.setImage(image);
		Label lblMessage = (Label) parent.lookup(".lblMessage");
		lblMessage.setText("메시지");
	...
	protected void handleCustom() {
		 ...
		 * StageStyle
		 * DECORATED : 일반적 윈도우 스타일, 흰배경, 제목줄에 장식(아이콘, 타이틀, 축소, 복원, 닫기) -> 기본
		 * UNDECORATED 흰배경, 제목줄 X
		 * TRANSPARENT 투명 배경, 제목줄 X
		 * UNIFIED DECORATED와 같이만 내용물의 경계선 X
		...
		TextField textTitle = (TextField) parent.lookup("#textTitle");
		TextArea textContent = (TextArea) parent.lookup("#textContent");
	...
	public void handleSave(ActionEvent event) throws Exception{
		...
		// textArea에 입력된 내용을 FileChooser를 이용하여 선택된 파일에 출력
		FileChooser fileChooser = new FileChooser();
		fileChooser.getExtensionFilters().addAll(new ExtensionFilter("Text Files","*.txt"),new ExtensionFilter("Image Files","*.png","*.jpg","*.jpeg","*.gif")
				,new ExtensionFilter("Media Files","*.wav","*.mpg","*.mp4","*.avi"),new ExtensionFilter("All Files","*.*"));
