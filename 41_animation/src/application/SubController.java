	...
	public void initialize(URL location, ResourceBundle resources) {
		btnPrev.setOnAction(event->{
			...
			Timeline timeLine = new Timeline();
																	  // 투명도 0~1
			KeyValue keyValue = new KeyValue(subWrap.opacityProperty(),0);
			...
			timeLine.getKeyFrames().add(keyFrame);
												// 720도 만큼 회전
			keyValue = new KeyValue(subWrap.rotateProperty(),720);
			keyFrame = new KeyFrame(Duration.millis(1000),keyValue);
			timeLine.getKeyFrames().add(keyFrame);
			
			keyValue = new KeyValue(subWrap.scaleXProperty(),0);
			keyFrame = new KeyFrame(Duration.millis(1000),keyValue);
			timeLine.getKeyFrames().add(keyFrame);
			
			keyValue = new KeyValue(subWrap.scaleYProperty(),0);
			keyFrame = new KeyFrame(Duration.millis(1000),keyValue);
			timeLine.getKeyFrames().add(keyFrame);
